...
local function to_binary(number)
	...
	local pad = 4 - (#binary % 4)
	if pad ~= 4 then
		binary = string.rep("0", pad) .. binary
	end
	-- Insert space every 4 bits
	binary = binary:gsub("(%x%x%x%x)", "%1 "):gsub("%s+$", "")
	return "0b" .. binary
end
	...
	if word then
		...
		if number then
			...
			elseif string.match(word, "^0x") then
				...
				edits.replace_word_under_cursor_with(to_hex(number))
			end
		end
	end
end
...
local function display_other_representation()
	...
	if number and string.match(base, "dec") then
		api.nvim_buf_set_virtual_text(0, ns_id, row, { { to_binary(number) .. " " .. to_hex(number), "Comment" } }, {})
	elseif number and string.match(base, "hex") then
		api.nvim_buf_set_virtual_text(
			0,
			ns_id,
			row,
			{ { tostring(number) .. " " .. to_binary(number), "Comment" } },
			{}
		)
	elseif number and string.match(base, "bin") then
		api.nvim_buf_set_virtual_text(0, ns_id, row, { { tostring(number) .. " " .. to_hex(number), "Comment" } }, {})
	else
		clear_virtual_text()
	end
end
...
local function convert_selected_base(target_base, toggle)
	...
	if
		...
		and start_pos[3] == end_pos[3]
		and first_line == cursor_line
		and start_pos[3] == cursor_col
	then
		local word = edits.get_word_under_cursor()
		if word then
			local number, _ = parse_number(word)
...
function M.setup(opts)
	...
	api.nvim_create_user_command("NibblerToggle", function()
		convert_selected_base(nil, true)
	end, {
		nargs = "?",
		range = true,
		desc = "Toggles between binary, decimal, and hexadecimal representations",
	})
	api.nvim_create_user_command("NibblerToHex", function()
		convert_selected_base("hex", false)
	end, {
		nargs = "?",
		range = true,
		desc = "Converts a number to its hexadecimal representation",
	})
	api.nvim_create_user_command("NibblerToBin", function()
		convert_selected_base("bin", false)
	end, {
		nargs = "?",
		range = true,
		desc = "Converts a number to its binary representation",
	})
	api.nvim_create_user_command("NibblerToDec", function()
		convert_selected_base("dec", false)
	end, {
		nargs = "?",
		range = true,
	...
	api.nvim_create_autocmd({ "CursorMoved", "CursorMovedI" }, {
		...
		callback = display_other_representation,
	})
end
